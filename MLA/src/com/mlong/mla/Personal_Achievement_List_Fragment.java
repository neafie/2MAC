...
public class Personal_Achievement_List_Fragment extends BaseFragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        Personal_Achievement_List_Fragment myfrag = this;
        Bundle mybundle;
		...
		if(cursor != null) {
            if(cursor.getCount() > 0) {
                ...
                do {
                	...
                	if(cursor.getString(cursor.getColumnIndex(PersonalDB.COLUMN_ACHKEY)) != null)
                	{
                		Item myitem = new Item();
                		aname = cursor.getString(cursor.getColumnIndex(PersonalDB.COLUMN_ACHNAME));
                		dname = cursor.getString(cursor.getColumnIndex(PersonalDB.COLUMN_DESCRIPTION));
                		trophy = cursor.getInt(cursor.getColumnIndex(PersonalDB.COLUMN_ISCOMPLETED))>0;
                		time = cursor.getLong(cursor.getColumnIndex(PersonalDB.COLUMN_TIMEFRAME));
                		achkey = cursor.getInt(cursor.getColumnIndex(PersonalDB.COLUMN_ACHKEY));
                		iconpath = cursor.getString(cursor.getColumnIndex(PersonalDB.COLUMN_ICON));
	...
	public void setPointTotal()
	{
        ...
        if(cursor != null) {
            if(cursor.getCount() > 0) {
                ...
                do {
                	//can take out if later when add not null to database
                	if(cursor.getString(cursor.getColumnIndex(PersonalDB.COLUMN_POINTS)) != null)
                	{
                		getpoint1 = cursor.getInt(cursor.getColumnIndex(PersonalDB.COLUMN_POINTS));
                		if(getpoint1 >=0)
                		{
        ...
        if(cursor != null) {
            if(cursor.getCount() > 0) {
                ...
                do {
                	...
                	if(cursor.getString(cursor.getColumnIndex(PersonalDB.COLUMN_POINTS)) != null)
                	{
                		getpoint2 = cursor.getInt(cursor.getColumnIndex(PersonalDB.COLUMN_POINTS));
	...
	public void setAchTotal()
	{
        ...
        if(cursor != null) {
            if(cursor.getCount() > 0) {
                ...
                do {
                	//can take out if later when add not null to database
                	if(cursor.getString(cursor.getColumnIndex(PersonalDB.COLUMN_ACHNAME)) != null)
                	{
                		ach_total++;
                	}
                } while (cursor.moveToNext());
	...
	public boolean getiscomplete(int achkey)
	{
        ...
        if(cursor != null) {
            if(cursor.getCount() > 0) {
                ...
                do {
                	//can take out if later when add not null to database
                	if(cursor.getString(cursor.getColumnIndex(PersonalDB.COLUMN_ISCOMPLETED)) != null)
                	{
                		completed = cursor.getInt(cursor.getColumnIndex(PersonalDB.COLUMN_ISCOMPLETED))>0;
                	}
                } while (cursor.moveToNext()); 
            }
        }
        ...
        if(cursor != null) {
            if(cursor.getCount() > 0) {
                ...
                do {
                	...
                	{
                		ofcompleted = cursor.getInt(cursor.getColumnIndex(PersonalDB.COLUMN_NUMBEROFCOMP));
                	}
                } while (cursor.moveToNext());
            }
        }
